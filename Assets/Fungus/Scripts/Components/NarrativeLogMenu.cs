...
namespace Fungus
{
    ...
    public class NarrativeLogMenu : MonoBehaviour 
    {
        ...
        protected virtual void Start()
        {
            ...
            //Clear LogContent
            for(int i = 0;i<narrativeLogView.content.childCount;i++)
            {
                Destroy(narrativeLogView.content.GetChild(i).gameObject);
            }
        ...
        {
            Debug.Log("OnWriterState "+ writerState.ToString());
            ...
            if (writerState == WriterState.End)
            {
                ...
                if (sd != null)
                {
                    Debug.Log("OnNarrativeAdded");
                    GameObject g = Instantiate(LogContentPrefab, Vector3.zero, Quaternion.identity, narrativeLogView.content);
                    NarrativeLogView nlv = g.GetComponent<NarrativeLogView>();
                    nlv.NameText.text = sd.NameText;
                    nlv.ContentText.text = sd.StoryText;
                    if (sd.NameText.Length == 0)
                        nlv.NameText.transform.parent.gameObject.SetActive(false);
                }
            }
        }
        ...
        {
            if (narrativeLogView.enabled && narrativeLogView.viewport.childCount>0)
            {
